...
function peco_cd_history() {
    ...
    # [ $destination ] && cd ${destination/#\~/${HOME}}
    if [ $destination ]; then 
        cd ${destination/#\~/${HOME}}
    else
        :
...
function peco_insert_cd_history() {
    local destination=$(peco_get_destination_from_history)
    if [ $? -eq 0 ]; then
...
zle -N peco_insert_cd_history
# }}}

# C-p d j またはC-jでディレクトリに cd
# C-p d i でディレクトリを挿入

if [ `uname | grep -v 'CYGWIN'` ]; then
    bindkey '^pdj' peco_cd_history
    bindkey '^j' peco_cd_history
    bindkey '^pdi' peco_insert_cd_history
...
# 2015/02/27
# pretzo git alias: https://github.com/sorin-ionescu/prezto/blob/master/modules/git/alias.zsh 
